...
This checkout is the `blaikooz/vinodex-swift` package. Per
[KNOWN-ISSUES.md](KNOWN-ISSUES.md#repo-layout) it is assembled from the
`blaikooz/vinodex` monorepo by `scripts/publish-swift.mjs`, which would overwrite
anything committed here directly. **Confirm the current delivery path before
starting a batch of fixes** — if the publish step is still live, a merged PR here
does not survive it.

Paths map almost one-to-one between the two repos, so a fix written here applies
cleanly on either side:

| Here | In the monorepo |
|---|---|
| `Sources/`, `Tests/`, `Package.swift`, `xtool.yml` | same name under `ios/` |
| `shared/`, `scripts/`, `pixelflags/` | identical path |
...
- [ ] **M34** · ci · no CI at all — the Linux-ready test suite never runs automatically · repo root → GitHub Actions running `swift test` on push/PR
  - **Where it lands:** the workflow belongs in the monorepo, or as a gate on the publish step.
