...
struct AppVersionTests {
    /// Exactly three dot-separated numeric components, nothing else.
    ///
    /// The count *was* deliberately unpinned, because the strings had run to four
    /// and five parts. That freedom is what let three concurrent branches invent
    /// 0.4.2.1.1 and 0.4.2.1.2 for overlapping work, so from 0.4.3 the scheme is
    /// three components and this test is the thing that keeps it there — see the
    /// note in `AppVersion.swift`.
    ///
    /// Three is also the most `CFBundleShortVersionString` accepts, so a version
    /// that fails here could never be stamped into a signed bundle either. A
    /// stray `v`, a trailing dot or a `-beta` suffix would all reach the engraved
    /// nameplate verbatim, and none of them belong there.
    @Test("the version is three dot-separated numbers")
    func versionShape() {
        let parts = AppVersion.fallback.split(separator: ".", omittingEmptySubsequences: false)
        #expect(
            parts.count == 3,
            "\(AppVersion.fallback) has \(parts.count) components; the scheme is three from 0.4.3 on"
        )
        for part in parts {
    ...
    func realBundledWins() {
        ...
        // `resolve` reports what the bundle says without judging its shape — the
        // three-component rule is enforced on our own constant, above, not on a
        // value some future signing pipeline hands us.
    ...
    /// else — a second one added at the call site would read `vv0.4.3`.
