...
public struct CountryScreen: View {
    ...
    /// The flag plate. Only the portrait and the shelf control are this
    /// screen's business — the title's own treatment, hyphenation and the 18pt
    /// inset that keeps its hard shadow off the moulding (0.7.1, **A4**), now
    /// lives inside `DexHero` (AUDIT **M28**) rather than in the four
    /// hand-rolled copies of this plate that each had to be fixed separately.
    ...
    /// Headed **ALL GRAPES**, not NOTABLE GRAPES (0.6.7, B1): the heading
    /// follows the list, and the list is now the union of the regions' notable
    /// grapes with everything the catalog attributes to the country — see
    /// `grapeEntries` for why the narrower one was lossy.
    private var grapesSection: some View {
        let all = grapeEntries
        let shown = showsAllGrapes ? all : Array(all.prefix(3))
        return DexSection("ALL GRAPES", symbol: "list.bullet") {
            VStack(spacing: 8) {
