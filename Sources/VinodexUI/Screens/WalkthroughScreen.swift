...
public struct WalkthroughScreen: View {
    ...
    /// hierarchy.
    ///
    /// **Every button that moves `index` also sets `copyFocused`** (AUDIT
    /// **M48**). A step change is only *announced* because focus lands on the
    /// card that now carries the ordinal — see the note on `copy` for why the
    /// progress bar's value cannot do it. SHOW ME and DONE leave the page, so
