...
public struct TastingQuizScreen: View {
    ...
    private func choose(_ id: String, in question: QuizQuestion) {
        ...
        // **One voice per answer** (0.6.9, G1). This used to pair the result
        // sting with `screenTap()`/`select()`, both of which play Warm Ping
        // since 0.6.8 (J1) — so every answer fired two authored sounds a few
        // milliseconds apart, on the one moment in the app that most needs a
        // single unambiguous voice. `Haptics.answer(correct:)` owns the pairing
        // now, at the choke point rather than here, which is also the half of
        // AUDIT **L38** that survives the move: what an answer says is the
        // outcome's business, not the generic touch's. The buzz it plays is
        // J1's impact/selection split rather than L38's success/warning pair.
        //
        // **L43** is spent rather than dropped. It was written when
        // `Sounds.wrong()` was an empty stub — the wrong branch had no sound to
        // play, so the warning buzz carried it alone — and 0.6.8's J3 landed
        // the authored incorrect-answer sting that stub was holding the call
        // site open for. `answer(correct:)` plays it.
