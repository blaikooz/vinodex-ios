...
struct DexSearchField: UIViewRepresentable {
    ...
    /// The eight stored settings, as one model (arch **A17**). Read for the
    /// LCD mode below, which used to come from the `LcdMode.current` static —
    /// the last of **M13**'s residue.
    ...
    ///
    /// Read from `settings` rather than the `LcdMode.current` static (arch
    /// **A17**). The mechanism is unchanged and is not observation: a
    /// representable has no `body`, so what repaints this field is the parent
    /// re-rendering and SwiftUI calling `updateUIView` — which is what
    /// **M13** established. What changed is that the value now comes from the
    /// same model the parent is observing, so the two cannot read different
    /// modes in the same frame.
    private func applyColors(to field: UITextField) {
        let accent = UIColor(settings.lcdMode.accent)
