...
// The title panel, split out of DeviceChassis.swift (AUDIT **M30**). It was
// the single largest type in that file after the chassis itself, and the one
// with the most intricate reasoning behind it — the doc comments below are the
// record of several separate attempts. Nothing changed in the move itself;
// what has changed since is upstream's own work on the panel — 0.6.9's D1-D4
// retiring the scroll, 0.7.1's B1-B3 scripting what it says, and the pixel
// dissolve of 0.8.1 H2 / 0.8.3-0.8.5 — carried across to here rather than left
// behind in the file it was split out of.
...
/// the end gradient and the scroll clock as code nothing reaches, and every one
/// of those is a thing this panel has previously lost a round to (audit M8, M9,
/// and the 0.6.6 C2 fade). Gone with it: `segments`, `pointsPerSecond`,
/// `copyWidth`, `gap`, `symbolSize`, `staticLabel` and `DexMetrics.marqueeFade`.
/// M9's finding goes with them and is worth keeping in one line, because
/// `StretchedWordmark` still cites it: `UIFont` metrics and SwiftUI's own `Text`
/// layout do not agree, the error scales with the string's length, and a run
/// measured from its own laid-out geometry cannot disagree with itself.
///
...
struct MarqueeBanner: View {
    ...
    /// Zero *everywhere* since 0.7.6 (A1) retired the corner buttons; the
    /// parameter survives because "keep your ends clear" is worth having the
    /// next time something is drawn over the panel, and it costs one defaulted
    /// argument. See `DexMetrics`, where that arithmetic is written down.
    ...
    // The panel's phosphor follows the shell — see `ChassisSkin.marqueeText` —
    // unless the workshop has fitted a different one (0.7.3, B1). Two reads,
    // resolved together by `skin` below.

    /// The eight stored settings, as one model (arch **A17**): the shell comes
    /// from `settings.chassisSkin` rather than from another `@AppStorage`
    /// declaration of the same key with its own literal default, so the panel
    /// cannot hold a different opinion about the skin from the chassis it is
    /// mounted in.
    var settings: AppSettings = .shared

    /// The marquee axis (0.7.3, B1) — the one part override this view can see.
    ///
    /// Still an `@AppStorage`, and not a ninth property on `AppSettings`: the
    /// workshop's `DeviceAxis` keys are a separate system from the eight
    /// settings A17 models, and `ChassisButton` and `MarqueeLampChooser` read
    /// their own axes exactly this way. KVO-backed, so the panel repaints the
    /// instant the workshop writes a colour — which is what makes the device
    /// itself the builder's live preview. Empty resolves to "whatever the shell
    /// says", so a device nobody has customised is byte-identical to 0.7.2's.
    ...
    ///
    /// Its job changed twice. It used to choose between the scrolling and still
    /// forms; 0.6.9's D1 made the still form the only one. What is left for it
    /// to gate is the *dissolve* — see `change(to:)`, which swaps it for a
    /// cross-fade. The greeting cycle it also suspended is
    /// `DeviceChassis.runScript()` since 0.7.1 (B1-B3), and reads the setting
    /// for itself.
    @Environment(\.accessibilityReduceMotion) private var reduceMotion

    private var skin: ChassisLook {
        ChassisLook(skinRaw: settings.chassisSkin.rawValue, marquee: partMarquee)
    }
    ...
    /// `PulseGlow` makes the same distinction — the reduced branch settles on
    /// the most informative still state rather than simply refusing to update.
    /// (AUDIT M18)
    ...
    ///    rather than to a word chopped mid-glyph. (AUDIT M18 asked for exactly
    ///    this backstop on the still label; D1 made the still label the only
    ///    one, and D2 deepened the floor to fit the wrapped form.)
    ///
    ...
    private var title: some View {
        if fadeStart == nil {
            ...
            // reason `DataWave` sets out in `SettingsControls`: `Canvas` has no
