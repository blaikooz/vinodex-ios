...
struct DeviceBackPlate: View {
    ...
    init(db: WineDatabase = .shared, onReturn: (() -> Void)? = nil) {
        self.db = db
    ...
    var body: some View {
        ...
        // B3). It was a swipe anywhere on this surface — "the plate is a
        // surface, not a button" — and that had to go for two reasons: it
        // fought the stamps' own touches, and I1 gives the *LCD* an app-wide
        // back swipe while I2 excludes this plate, so a swipe here would be the
        // same movement meaning two different things depending on which face is
        // up. What survives of the old note is the half that was never about
        // the gesture: `DeviceChassis` still owns `isFlipped`, and the plate
        // reports up through `onReturn` rather than reaching for it.
        ZStack {
