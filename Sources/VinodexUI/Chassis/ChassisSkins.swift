...
// The skin catalogue's colours — five devices rather than one device in five
// colourways (v0.4.2.1), each carrying its own accent ramp, control livery and
// marquee phosphor.
//
// `ChassisSkin` itself — the cases, the persisted raw values, the storage key,
// the labels, the emblems and `next` — moved to
// `Sources/VinodexCore/ChassisSkin.swift` (arch **A6**). What is left here is
// the half that needs `Color`, plus the two part descriptions it is built from.
// The file was split out of DexTheme.swift by AUDIT **M30**; nothing in either
// move changed a value.
