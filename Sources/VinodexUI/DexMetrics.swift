...
// Every fixed dimension the device is drawn at, and the UI-scale factor
// applied to them.
...
public enum DexMetrics {
    ...
    /// lines of type.
    public static var rowGlyph: CGFloat { 28 * UIScale.current.factor }
    /// The gutter a `rowGlyph` sits in. Wider than the glyph so a tall drawing
    /// and a wide one both centre in the same column.
    public static var rowGlyphGutter: CGFloat { 36 * UIScale.current.factor }
    public static var tileGlyph: CGFloat { 52 * UIScale.current.factor }
