...
///
/// `@MainActor` because it reads `PixelArtLoader.subdirectories`, and the
...
struct DexAssetAudit {
    ...
    static func run(_ db: WineDatabase = .shared) -> [Surface] {
        ...
        return [
            ...
            surface("art", ids: artStems.sorted()) { exists($0, in: PixelArtLoader.subdirectories) },
            surface("flavorArt", ids: stems(icons.flavorArt)) {
                exists($0, in: ["Resources/FlavorArt"])
            },
    ...
    private static func hasIconVariants(_ slug: String) -> Bool {
        [slug, "\(slug)@2x", "\(slug)@3x"].allSatisfy {
            DexResources.url(named: $0, ext: "png", subdirectory: "Resources/Icons") != nil
        }
    }

    private static func exists(_ stem: String, in subdirectories: [String]) -> Bool {
        subdirectories.contains {
            DexResources.url(named: stem, ext: "png", subdirectory: $0) != nil
        }
    }
}
#endif
