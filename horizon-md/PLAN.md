...
| 0.8.91 | `vinodex-0.8.91.md`, sections A-H, plus section I added on the user's ask alongside the written spec and treated as first-class | **TWO READERS, TWO SPACES, AND THE BUBBLES WERE NEVER THE PROBLEM.** The headline pair is the boot screen and the first run. **§G3** mounts the delivered `VinodexBootView` and retires `BootScreen.swift` with it, so there is one boot path rather than two; the *composition* is the new file and the *content* is not -- `BootSequence` still owns the lines and the clock and `BiosChrome` still owns the strings, the schedule is absolute against one `start` instant so the whole screen is bounded by `longestUntouched` (5.4s, the number `neverTraps` already pins), and the mockup's `v1.0.0` was disobeyed for the third release running because that literal is on `AppVersion.placeholders`. The delivered `slowMotion: true` default ran roughly twice the ceiling and ships off. Assets: the shipped two-layer pixel V for the mark and `Logo/vinodex-logo.png` -- in the tree since 0.7.5 with no call site -- for the corner badge; the wordmark stays the retro face, because there is no wordmark PNG and a new one is a pipeline change. **§I1 is a coordinate-space bug, not a layout one.** `DeviceChassis` is a `GeometryReader` laid out *inside* the safe area (it reads `safeAreaInsets.top` on purpose, to use the island band); `RootView`'s `overlayPreferenceValue` reader inherited that space and resolved every anchor in it, while `CoachmarkOverlay` carried a *second* `GeometryReader` with `.ignoresSafeArea()` and drew in full-screen space. One top inset apart, on a `size.height` about ninety points taller than the rects were measured against -- which is all three symptoms the item reports (off-target by a constant, over the subject when the offset pushed a below-bubble up, off-screen for near-bottom targets) from one cause. Fixed by having **one** reader: the overlay takes `canvas` as a parameter, so there is no longer a pair that can drift. Three more found underneath it: the preference reduce kept the *last* publisher and its own comment claimed that made the USER screen's PASSPORT row beat the chassis button -- the footer is the later child, so it did the opposite, and it keeps the first now; a clamped rect could be empty and still `.some`, so a `.listingRow` scrolled out of a `LazyVStack` drew a zero-size lit ring at a screen edge with a caret pointing into it (`minimumTarget` makes that 'no target', a state the bubble already knows); and the placement was a half-screen guess pinned by a spacer capped at 62% of the window, which detached the bubble from its subject while the caret went on tracking it. It is a fit test over a *measured* bubble height now, and it drops the arrow rather than lying with it when neither side fits. **§I2/§G1**: the coachmark half was already Vino and the *map* half was not -- `WalkthroughScreen` was a title over a paragraph in a plain panel, so a new user met a character on the guided run and a form on the tour that precedes it. `WalkthroughStep` gains an `expression`, all twelve steps get a face, and the copy moves into the same portrait-and-bubble the other two surfaces use; `WalkthroughVoiceTests` holds every face in use and pins that both halves end on the same one. Portraits 52->64, 64->84, 46->58, and the coachmark's `STEP n OF m` becomes `PROF. VINO`. **§D1 is the fourth pass at the Home cap and this one measured the other three.** 0.8.7's E1 read a depth table (`back` 11, `user` 11, `settings` 12, `home` 12 with '2,079 pixels at 13 and beyond') as a shadow separating from a cap. Re-measured, the band it removes from `home` is the *same lower lip* `back` and `user` keep -- rows 222/230/238/246 run x 47-193, 63-179, 76-165, 93-146 on `home` against 48-195, 63-185, 77-167, 94-148 on `back`, four identical rows of skirt tapering identically. What differs is thickness: 21 opaque pixels across that span on `home` where `back` and `user` carry 120 and 119, so the path down through it is narrow and long and a geodesic-*distance* rule scores it as far away. The trim was a no-op on three caps and destructive on the fourth, so `outlineReach` is retired and the largest-connected-component rule (0.8.6, B1) is the whole silhouette rule again. **§B1's premise was wrong in the app's favour**: Saved and Wanted are not new user states, `Shelf` has had all three since the user panel shipped and `EntryDetailScreen` has had all three setters, so the item reduces to exposing them -- a `.shelf` `ChipFacet` plus a `ShelfMembership` snapshot, because it is the first facet whose predicate needs state outside the `WineEntry` and `matches` is pure, Foundation-only and not `@MainActor`. **§C2 and §C3 had no asset**: the bell did not exist anywhere in `art/` and the label scanner existed only as a button face, so both were drawn on a 24x24 grid in the glyph family's palette and imported -- a different hand in the same register, at 22-84pt. `hammer`, `seal` and `mail` come off `UIGlyph.unwired`, which is the move that set exists to make legible. **§H1** rewrote all 29 changelog entries to one sentence a change (26,931 characters of notes to 15,791, longest note 340 to 139) and replaced the editorial headlines -- A DIAL, NOT A LIST, PROSECCO IS NOT A ROSE -- with headings that say which release you are looking at. | 446, untouched; `firmware.ts` rewritten, two new glyph masters | 738 tests, `npm run generate` + `find-missing-refs` (zero dangling) + `icons:verify` green, clean xtool build, deployed |
